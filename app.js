...
app.post('/api/restartBot', (req, res) => {
  ...
  if (restartInterval) {
    // Adjust the autoRestart timeout with the provided interval
    const timeoutMs = restartInterval * 60 * 1000; // Convert minutes to milliseconds

    res.json({ success: true, message: `Bot will restart in ${restartInterval} minutes.` });

    // Optionally, you can immediately trigger a restart after the timeout
    setTimeout(() => {
      process.exit(1);
